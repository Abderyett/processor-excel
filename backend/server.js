...
// ────────────────────────────────────────────────────────────────
//  NEW: Extract date from filename and determine which file is newer
// ────────────────────────────────────────────────────────────────
const extractDateFromFilename = (filename) => {
	// Extract date pattern like "26_06" or "27_06" from filename
...
const determineNewerFile = (file1, file2) => {
	const date1 = extractDateFromFilename(file1.originalname);
	const date2 = extractDateFromFilename(file2.originalname);

	if (!date1 || !date2) {
		// If we can't extract dates, use file modification time or default to first file
		return file1;
	}

	return date1 > date2 ? file1 : file2;
};

// ────────────────────────────────────────────────────────────────
//  NEW: Compare and Clean functionality
// ────────────────────────────────────────────────────────────────
const compareAndClean = (files) => {
	console.log('Processing Compare and Clean...');

	if (files.length !== 2) {
		throw new Error('Compare and Clean requires exactly 2 files');
	}
	...
	console.log(`Newer file: ${newerFile.originalname}`);
	console.log(`Older file: ${olderFile.originalname}`);
	...
	// Extract all email addresses from older file
	const olderEmails = new Set();
	olderWb.SheetNames.forEach((sheetName) => {
		const rows = XLSX.utils.sheet_to_json(olderWb.Sheets[sheetName], { defval: '' });
		rows.forEach((row) => {
	...
	console.log(`Found ${olderEmails.size} unique emails in older file`);

	// Process newer file and remove duplicates
	const cleanedData = [];
	let duplicatesRemoved = 0;
	let totalRows = 0;

	newerWb.SheetNames.forEach((sheetName) => {
		const rows = XLSX.utils.sheet_to_json(newerWb.Sheets[sheetName], { defval: '' });
	...
	console.log(`Cleaned data contains ${cleanedData.length} rows`);

	// Create new workbook with cleaned data
	const cleanedWb = XLSX.utils.book_new();
	const cleanedWs = XLSX.utils.json_to_sheet(cleanedData);
	XLSX.utils.book_append_sheet(cleanedWb, cleanedWs, 'Cleaned Data');

	// Generate filename with "clean" suffix
	const baseFilename = newerFile.originalname.replace(/\.(xlsx?|csv)$/i, '');
	const extension = newerFile.originalname.match(/\.(xlsx?|csv)$/i)?.[0] || '.xlsx';
	const cleanedFilename = `${baseFilename}_clean${extension}`;
...
// ────────────────────────────────────────────────────────────────
//  Helper to make safe Excel attachments
// ────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────
//  POST  /api/process (updated to handle compare and clean)
// ────────────────────────────────────────────────────────────────
app.post('/api/process', upload.array('files'), async (req, res) => {
	try {
		...
		// NEW: Handle Compare and Clean
		if (options.compareAndClean) {
			if (files.length !== 2) {
			...
			summary.push(
				`Compare & Clean: ${result.duplicatesRemoved} duplicates removed from ${result.totalOriginalRows} rows. ` +
					`Final file: ${result.rowCount} rows (${result.newerFileName} cleaned against ${result.olderFileName})`
			);
		} else {
		...
		const emailSubject = options.compareAndClean
			? `Cleaned Excel file – ${new Date().toLocaleDateString()}`
			: `Processed Excel files – ${new Date().toLocaleDateString()}`;

		const emailBody = options.compareAndClean
			? `<p>Your files have been compared and cleaned:</p><ul>${summary
					.map((s) => `<li>${s}</li>`)
					.join('')}</ul>`
			: `<p>Your files have been processed:</p><ul>${summary.map((s) => `<li>${s}</li>`).join('')}</ul>`;
		...
		res.json({
			...
			details: summary,
			isCompareAndClean: !!options.compareAndClean,
		});
	} catch (err) {
		console.error(err);
